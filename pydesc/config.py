...
class Branch:
    ...
    def set(self, param_name, value, default_value=None):
        ...
        fset = lambda self, value: self._set_p(param_name, value, default_value)
...
class ConfigManager:
    ...
    def new_branch(branch_name):
        setattr(
            ConfigManager,
            branch_name,
            Branch(ConfigManager.__name__ + "." + branch_name),
        )
                    ...
                    if len(v) > print_limit > 0:
                        try:
                            ...
                            print(
                                ("\t" * (k.count(".") - 1)),
                                "-" + k.rsplit(".", 1)[1] + " =",
                                dict(islice(iter(v.items()), print_limit)),
                                "... <<" + str(len(v) - print_limit) + " more>>",
                            )
                            continue
                except:
                    pass
