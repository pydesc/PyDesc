...
class Attachment(object):
    ...
    def attach(self, substructure):
        ...
        if substructure not in self:
            ...
            raise Warning(
                "Object %s is already attached to %s"
                % (str(substructure), str(self.main_object))
            )
...
class Entry(object):
    ...
    def __init__(
        self, pymol_object_name, pymol_state, attachment, selection_flag=False
    ):
...
class Registry(object):
    ...
    def add(structure_obj, name, state=1, selection_flag=False, structure_flag=False):
        ...
        if selection_flag:
            ...
            if (
                structure_obj.derived_from in Registry.objects
                and structure_obj.derived_from != structure_obj
            ):
            ...
            if structure_obj not in Registry.objects:
                if attachment is None:
                    Registry.objects[structure_obj] = Entry(
                        name, state, Attachment(structure_obj), selection_flag
                    )
                else:
                    Registry.objects[structure_obj] = Entry(
                        name, state, attachment, selection_flag
                    )
            else:
...
def load_structure_decorator(current_load_structure):
    ...
    def wrapped_load_structure(self, code, *args, **kwargs):
        ...
        for structure_index, structure_obj in enumerate(list_of_structures):
            cmd.read_pdbstr(
                structure_obj.create_pdb_string(transformed=False).read(),
                structure_obj.name,
                state=structure_index + 1,
...
def _update_transformation(self):
    ...
    try:
        if (
            cmd.get_title(*Registry.get(self.attachment.main_object, "name", "state"))
            == "PD"
        ):
            matrix = cmd.get_object_matrix(
                Registry.get(self.attachment.main_object, "name")
            )
            self.reset()
...
def transform_object(self, trtm):
    cmd.transform_object(
        Registry.get(self, "name"), trtm.convert_to_flat_pymol_matrix()
    )
...
def w_load(app):
    ...
    pdb_code = tkinter.simpledialog.askstring(
        "PyDesc", "Please enter a 4-char pdb code:", parent=app.root
    )
    try:
        loader.load_structures(pdb_code)
    except:
        tkinter.messagebox.showerror(
            "Invalid code", "The code you entered is incorrect:" + pdb_code
        )
        raise
...
def __init__(self):
    ...
    self.menuBar.addmenuitem(
        ...
        command=lambda x=self: w_load(x),
    )
...
if __name__ == "pydesc.descmol":
    ...
    structure.AbstractStructure.rotate = rotate_decorator(
        structure.AbstractStructure.rotate
    )
    structure.AbstractStructure.translate = translate_decorator(
        structure.AbstractStructure.translate
    )
    structure.AbstractStructure.transform_object = transform_object
    # LOAD STRUCTURE
    structure.StructureLoader.load_structures = load_structure_decorator(
        structure.StructureLoader.load_structures
    )
    # DESCRIPTOR
