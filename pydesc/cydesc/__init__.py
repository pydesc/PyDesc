...
import pydesc.contactmap as contactmap
...
def load_library(name):
    ...
    for ext in extensions:
        try:
            if req is not None:
                fname = pkg_resources.resource_filename(req, '/lib/lib%s%s' % (name, ext))  # pylint: disable=E1101
            else:
                fname = os.path.join(os.path.dirname(os.path.dirname(pydesc.__file__)), 'lib/lib%s%s' % (name, ext))
        except KeyError:
            continue
...
class CInDelMeta(ctypes.Structure.__class__):
    ...
    def __new__(mcs, name, bases, nmspc):
        ...
        size = ctypes.sizeof(ctypes.Structure.__class__.__new__(mcs, name, bases, nmspc))
...
class CMer(ctypes.Structure):
    ...
    def __init__(self, m):  # pylint: disable=W0231
        ...
        self.points = ctypes.cast(
            (CPoint * self.n_points)(*points),  # converts <<points>> into a ctypes array
            ctypes.POINTER(CPoint)
        )  # casts an array of CPoint onto a POINTER (to CPoint)
        self.point_names = ctypes.cast((ctypes.c_char_p * self.n_points)(*point_names), ctypes.POINTER(ctypes.c_char_p))
...
class CStructure(ctypes.Structure):
    ...
    def __init__(self, struct):  # pylint: disable=W0231
        ...
        self.monomers = ctypes.cast((CMer * self.n_monomers)(*cmers), ctypes.POINTER(CMer))
...
class CContactMap(ctypes.Structure):
    ...
    def __init__(self, struct, contact_map):  # pylint: disable=W0231
        ...
        self.contacts = ctypes.cast(
            (CContact * self.n_contacts)(*contacts_list),  # converts <<contacts>> into a ctypes array
...
class CElement(ctypes.Structure):
    ...
    # python types are converted into integers according to this dictionary
    types_dict = {structure.ElementChainable: 1,
                  structure.ElementOther: 2}
...
class CDescriptor(ctypes.Structure):
    ...
    # python types are converted into integers according to this dictionary
    types_dict = {structure.ProteinDescriptor: 1,
                  structure.NucleotideDescriptor: 2}
    ...
    def __init__(self, descriptor):  # pylint: disable=W0231
        ...
        contact_map = contactmap.ContactMap(descriptor, contacts.DescriptorCriterion(descriptor))
        ...
        self.elements = ctypes.cast((CElement * self.n_elements)(*elements), ctypes.POINTER(CElement))
        self.central_element = descriptor.central_element.central_monomer.ind

    def __repr__(self):
        return '<CDescriptor for %s#%d:: n_elements:%d>' % (repr(self.structure), self.central_element, self.n_elements)
