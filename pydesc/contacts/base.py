...
class ContactCriterion(metaclass=ABCMeta):
    ...
    @abstractmethod
    def calculate_contact(self, mer1, mer2):
        pass
        # TODO: metaclass that checks if vectorized OR calc_contact is overridden

    def vectorized_calculate_contact(self, structure_obj):
        ...
        for mer1 in mers1:
            for mer2 in mers2:
                value = self.calculate_contact(mer1, mer2)
                contact_map[mer1.ind, mer2.ind] = value
...
class NotCriterion(ContactCriterion):
    ...
    def calculate_contact(self, mer1, mer2):
        value = self.criterion.calculate_contact(mer1, mer2)
        return self.mapping[value]

    def vectorized_calculate_contact(self, structure_obj):
        method = self.criterion.vectorized_calculate_contact
        contact_map = method(structure_obj)
        return (contact_map - 1) * -1 + 1  # revert values
...
class CombinedContact(ContactCriterion, metaclass=ABCMeta):
    ...
    @abstractmethod
    def calculate_contact(self, mer1, mer2):
        values = numpy.zeros(self.n_criteria)
        for n, criterion in enumerate(self.criteria):
            value = criterion.is_in_contact(mer1, mer2)
            if value == 0:
                return 0
            values[n] = value
        return values
    ...
    @abstractmethod
    def vectorized_calculate_contact(self, structure_obj):
        method = self.criteria[0].vectorized_calculate_contact
        contact_map = method(structure_obj)
        for criterion in self.criteria[1:]:
            method = criterion.vectorized_calculate_contact
            new_layer = method(structure_obj)
            contact_map = numpy.stack((contact_map, new_layer), axis=2)
        return contact_map
...
class ContactsConjunction(CombinedContact):
    ...
    def calculate_contact(self, mer1, mer2):
        values = super().calculate_contact(mer1, mer2)
        return numpy.min(values)

    def vectorized_calculate_contact(self, structure_obj):
        contact_map = super().vectorized_calculate_contact(structure_obj)
...
class ContactsExclusiveDisjunction(CombinedContact):
    ...
    def calculate_contact(self, mer1, mer2):
        values = super().calculate_contact(mer1, mer2)
        twos = numpy.count_nonzero(values == 2)
        ones = numpy.count_nonzero(values == 1)
        if twos == 1:
            return 2
        elif twos > 1:
            return 0
