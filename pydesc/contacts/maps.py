...
class ContactMapCalculator(object):
    ...
    def _get_dict_like_matrix(self):
        structure_length = self.structure.derived_from[-1].ind
        return dok_matrix((structure_length + 1, structure_length + 1), dtype=int)

    def _fill_interaction_matrix(self, interaction_function):
        """Return interaction matrix filled with values returned by given interaction function.

        Argument:
        interaction_function -- function taking mer1, mer2 and interaction matrix and returning None.
        Purpose of this function is to fill mtx with appropriate value in appropriate way.
        """
