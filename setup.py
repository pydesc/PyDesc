...
cmdclass = {'install_lib': install_lib, 'build_shlib': build_shlib,
            'develop': develop}
...
try:
    ...
    locations = [os.path.join(os.environ['HOME'], 'local'), '/usr/local',
                 '/usr']
...
compile_args = ['-Wall', '-Wno-unknown-pragmas', '-std=gnu99', '-mmmx',
                '-msse', '-msse3', '-D_GNU_SOURCE', '-m64',
                '-fopenmp']

compdesc_sources = ['compdesc.c', 'rmsd.c', 'alignment.c', '01_prelim.c',
                    '02_components.c', '03_build.c', '04_prune.c']
...
shlibs = [SharedLibrary(name='cydesc',
                        sources=['src/cstructures.c', 'src/util/arrays.c',
                                 'src/util/bitops.c', 'src/util/maps.c',
                                 'src/util/random.c'],
                        include_dirs=['src/include'],
                        libraries=['m'],
                        extra_compile_args=list(compile_args)
                        ),
          SharedLibrary(name='overfit',
                        sources=['src/overfit.c'],
                        include_dirs=['src/include'],
                        libraries=['m'],
                        extra_compile_args=list(compile_args)
                        ),
          SharedLibrary(name='fitdesc',
                        sources=['src/fitdesc.c', 'src/overfit.c'],
                        include_dirs=['src/include'],
                        libraries=['m'],
                        extra_compile_args=list(compile_args)
                        ),
          SharedLibrary(name='compdesc',
                        sources=compdesc_sources + ['src/overfit.c',
                                                    'src/util/maps.c',
                                                    'src/util/arrays.c'],
                        include_dirs=['src/include'],
                        libraries=['m'],
          ...
          SharedLibrary(name='cydesc_test',
...
if has_libdesc:
    ...
    shlibs.append(SharedLibrary(name='compdesc_test',
                                sources=['src/compdesc_test.c',
                                         'src/util/arrays.c'],
                                include_dirs=['src/include', libdesc_inc_path],
                                libraries=['m', 'desc', 'gen_avl', 'overfit',
                                           'desc_comp_domains', 'arrays',
                                           'png',
                                           'graph', 'motzkin'],
...
setup(name='PyDesc',
      version='0.x',
      description='Toolkit for analysis of biopolymer structures using local descriptors',
