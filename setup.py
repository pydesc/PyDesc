...
def discover_libdesc(path):
    """ Searches for libdesc includes in given path. Returns a boolean value and a path or an empty string. """
...
shlibs = [SharedLibrary(name='cydesc',
          ...
          SharedLibrary(name='compdesc',
                        sources=compdesc_sources + ['src/overfit.c', 'src/util/maps.c', 'src/util/arrays.c'],
                        include_dirs=['src/include'],
                        libraries=['m'],
                        extra_compile_args=list(compile_args)
                        ),
