/*
 * Copyright 2004-2010, 2017 Pawel Daniluk
 * 
...
#define P_OCT(x) {printf(#x ": %o ", x);}
#define P_LONG_INT(x) {printf(#x ": %lld ", x);}
#define P_POINT(x) {printf(#x ": %p ", x);}
#define __P_OCT_ARR(x, f, t, arr) printf(#x "[%d .. %d]: ", f, t); for(int __i=f; __i<=t; printf("%o ", arr[__i++]))
#define P_OCT_ARR(x,n) {typeof (*x) *__x=(x); __P_OCT_ARR(x, 0, ((n)-1), __x);}
#define P_OCT_ARR_L(x) {typeof (*x) *__x=(x); __P_OCT_ARR(x, 1, __x[0], __x);}
